...
% it looks, edit this file.
%
% WHICH sections print is a record fact, not a layout one. The `\cvpart` lines in
% the document below place the sections whose heading or setting this file
...
% or gives `printed: false`, prints nothing at all - no heading, no gap - so this
% line can stay. Reorder the CV by reordering these lines in the document below.
\newcommand{\cvpart}[2]{%
  \cvdeclare{#2}%
  \ifnum\csname cv#2Count\endcsname>0
  \ifnum\csname cv#2Printed\endcsname>0
  \section{\textbf{#1}}
  \vspace{-0.4mm}
...
% below are undefined for someone whose cv.yaml has no `supervision:` key, and
% must guard on \cv<Key>Printed too, so the record's `printed: false` holds here
% exactly as it does for a \cvpart section.
